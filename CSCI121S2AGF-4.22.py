...
def locateBee(filename):
    ...
    while aline:
        ...
        for words in values:
            ...
            newnewlist=[]
            if words=="bee":
                spot = [linecount,wordcount]
                newnewlist.append(spot)
                newlist.append(newnewlist)
        aline=filename.readline()
    filename.close()
    return (newlist)

#print(locateBee("bee.txt"))
...
#example4(locateBee("bee.txt"))

def example5(addresses, infile):
    f = open(addresses, 'r')
    #g = open(infile, 'r')
    lines = f.readline()
    ls = []
...
example5("locations.txt", "bee.txt")
#example4(ls)